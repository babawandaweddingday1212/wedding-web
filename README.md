...
- **定錨**：`#/album` 這類網址會開資訊頁並捲到對應區塊。由於頁面上方有 lazy 載入的照片與地圖 iframe，載完會把版面撐高、位置就跑掉了，所以 `scrollToSection()` 會盯著文件高度反覆對位，直到高度安定（或使用者自己動了捲軸）才停手。這個對位**只在錨點真的改變時才跑**：以前是只要網址有錨點就跑，於是任何一個 popstate（iOS 從背景回到前景、邊緣返回手勢）都會把正在看相簿的人拉回區塊頂端。
- **捲軸位置**：只有站內的程式導航（首頁 → 動畫 → 資訊頁）才會把捲軸拉回頂端。上一頁／前進／重新整理都交給瀏覽器自己還原 —— 那些情況下強制捲到頂端，使用者感受到的就是「網頁自己跳掉了」。
- **`#/album` 與 `#/info/groom` 是同一頁**：短網址不帶賓客身份，路由會沿用上一次看到的身份（`lastSide`），否則 key 會在 `info:` 與 `info:groom` 之間跳動，兩者被當成不同頁面，在它們之間來回就會把整個資訊頁重建一次。
- **不要把 `#album-book` 直接交給 page-flip**：它的 `destroy()` 是 `this.ui.destroy(), this.block.remove()` —— 會把你傳進去的那個元素整個從 DOM 移除。手機一轉成橫式就會跨過 700px 斷點觸發重建，容器連同相簿一起消失，之後每次 `build()` 都是在一個脫離文件的元素上跑，轉回直式也救不回來。所以每次重建都新建一個拋棄式的 `.album-stage` 交給它，`#album-book` 自己永遠留在文件裡。
- **相簿記得看到哪一張**：`albumFlipbook.js` 把目前那張的原始檔名存進 `sessionStorage`。重建的時機比想像中多 —— 手機轉成橫式一定會跨過 700px 的跨頁／單頁斷點，iOS 記憶體吃緊時也會把背景分頁整個丟掉重載。兩種模式的頁序長度不同（跨頁 78 頁、單頁 57 頁），所以是用檔名而不是索引對位。另外，還原位置時 `turnToPage()` 會非同步補送一次 `flip` 事件，那一次要忽略，否則會把記憶值往回帶一格。
